...
parser.add_argument("--input_path", default="data/")  # data location
parser.add_argument("--filter_size", type=int, default=10)  # with this you can scan other filter sizes
...
print(args)  # it's good to have this info in log file just in case
...
# Build a model as in 
# DeepVirFinder https://arxiv.org/pdf/1806.07810.pdf


inputs = Input(shape=(sequence_length, 5))  # 5 because we allow "ATGCN", even though there is no N-s in our data

first_conv = Conv1D(args.layer_sizes, args.filter_size, activation="relu")(inputs)
first_pool = GlobalMaxPooling1D()(first_conv)  # returns one value per filter
drop1 = Dropout(args.dropout)(first_pool)

fc_layer1 = Dense(args.layer_sizes, activation="relu")(drop1)
drop2 = Dropout(args.dropout)(fc_layer1)

final = Dense(1, activation="sigmoid")(drop2)
...
callbacks.append(
    ModelCheckpointAUROC(filepath=args.save_path + ".hdf5", validation_data=(val_seqs, val_labels), verbose=1,
                         save_best_only=True))
